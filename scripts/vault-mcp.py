...
def search(query):
    ...
    return sorted(out,key=lambda x:(-x["score"],x["id"]))[:10]
...
def run_engine(args,input_profile=None):
    if input_profile is None:
        r=subprocess.run([sys.executable,str(ENGINE),*args],cwd=ROOT,capture_output=True,text=True)
    else:
        with tempfile.TemporaryDirectory() as td:
            p=Path(td)/"profile.json"; p.write_text(json.dumps(input_profile),encoding="utf-8")
            r=subprocess.run([sys.executable,str(ENGINE),args[0],str(p),*args[1:]],cwd=ROOT,capture_output=True,text=True)
    if r.returncode not in (0,1,2): raise RuntimeError(r.stderr or r.stdout)
    return r

TOOLS=[
 {"name":"search_vault","description":"Search interactive effects by business purpose, section, runtime or signal.","inputSchema":{"type":"object","properties":{"query":{"type":"string"}},"required":["query"]}},
...
def call(name,args):
    if name=="search_vault": return search(args.get("query",""))
    if name=="get_effect_contract": return get_effect(args["effect_id"])
    if name=="select_interactive_effect":
        r=run_engine(["select","--section",args.get("section","hero"),"--performance-priority",args.get("performance_priority","normal"),"--asset-readiness",args.get("asset_readiness","limited")],args["profile"])
        return json.loads(r.stdout)
    if name=="get_effect_skill":
        r=run_engine(["skill",args["effect_id"]])
        if r.returncode: raise KeyError(args["effect_id"])
        return r.stdout
    raise KeyError(name)
...
def handle(msg):
    ...
    if method=="initialize":
        ...
        return response(i,{"protocolVersion":pv,"capabilities":{"tools":{"listChanged":False}},"serverInfo":{"name":"ai-ui-style-vault","version":"1.0.0"}})
    if method=="notifications/initialized": return None
    if method=="ping": return response(i,{})
    if method=="tools/list": return response(i,{"tools":TOOLS})
...
def serve():
    for line in sys.stdin:
        ...
        try: out=handle(json.loads(line))
        except Exception as exc: out=response(None,error={"code":-32700,"message":str(exc)})
        if out is not None:
